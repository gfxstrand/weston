...
rpir_view_destroy(struct rpir_view *view)
{
	...
	view->surface->view_count--;
	if (view->surface->view_count == 0 && view->surface->surface == NULL)
			    ...
			    pixman_region32_t *output_damage)
{
	...
	wl_list_for_each_reverse(wv, &compositor->view_list, link) {
		...
		surface = to_rpir_surface(wv->surface);
		if (surface->need_swap)

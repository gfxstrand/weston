			     ...
			     struct weston_surface *configured_surface)
{
	...
	mode.flags = 0;
	mode.width = surf_width * fsout->pending.surface->buffer_viewport.scale;
	mode.height = surf_height * fsout->pending.surface->buffer_viewport.scale;
	mode.refresh = fsout->pending.framerate;

	ret = weston_output_switch_mode(fsout->output, &mode,
					fsout->pending.surface->buffer_viewport.scale,
